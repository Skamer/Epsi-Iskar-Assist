...
-- ****************************
local AURA_EYE_OF_ANZU, AURA_EYE_OF_ANZU_ICON = GetSpellName(EIA.AuraEyeOfAnzuSpellID), GetSpellTexture(EIA.AuraEyeOfAnzuSpellID)
local AURA_PHANTASMAL_WINDS, AURA_PHANTASMAL_WINDS_ICON = GetSpellName(EIA.AuraPhantasmalWindsSpellID), GetSpellTexture(EIA.AuraPhantasmalWindsSpellID)
local AURA_PHANTASMAL_WOUNDS, AURA_PHANTASMAL_WOUNDS_ICON = GetSpellName(EIA.AuraPhantasmalWoundsSpellID), GetSpellTexture(EIA.AuraPhantasmalWoundsSpellID)
local AURA_PHANTASMAL_FEL_BOMB = GetSpellName(EIA.AuraPhantasmalFelBombSpellID)
local AURA_FEL_BOMB, AURA_FEL_BOMB_ICON = GetSpellName(EIA.AuraFelBombSpellID), GetSpellTexture(EIA.AuraFelBombSpellID)
local AURA_PHANTASMAL_CORRUPTION, AURA_PHANTASMAL_CORRUPTION_ICON = GetSpellName(EIA.AuraPhantasmalCorruptionSpellID), GetSpellTexture(EIA.AuraPhantasmalCorruptionSpellID)
local AURA_DARK_BINDINGS, AURA_DARK_BINDINGS_ICON = GetSpellName(EIA.AuraDarkBindingsSpellID), GetSpellTexture(EIA.AuraDarkBindingsSpellID)
local AURA_RADIANCE_OF_ANZU, AURA_RADIANCE_OF_ANZU_ICON = GetSpellName(EIA.AuraRadianceOfAnzuSpellID), GetSpellTexture(EIA.AuraRadianceOfAnzuSpellID)
...
-- *** Event Handlers ***
-- **********************
function EyeOfAnzuAssist:HandleCombatLog(event)
...
function EyeOfAnzuAssist:CreatePlayerFrame(categoryName, index)
  ...
  if MyClass == "PRIEST" then
    local spellName = GetSpellName(527)
    button:SetAttribute("spell2", spellName)
  elseif MyClass == "PALADIN" then
    local spellName = GetSpellName(4987)
    button:SetAttribute("spell2", spellName)
  elseif MyClass == "MONK" then
    local spellName = GetSpellName(115450)
    button:SetAttribute("spell2", spellName)
  elseif MyClass == "SHAMAN" then
    local spellName = GetSpellName(77130)
    button:SetAttribute("spell2", spellName)
  elseif MyClass == "DRUID" then
    local spellName = GetSpellName(88423)
    button:SetAttribute("spell2", spellName)
  elseif MyClass == "EVOKER" then
    local spellName = GetSpellName(365585)
    button:SetAttribute("spell2", spellName)
  end
